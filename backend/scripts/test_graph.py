...
def run_graph_test_case(label: str, account_id: str, expected_path: str) -> dict:
    ...
    print(
        f"   Step 1: Ingesting interaction and running graph (Thread: {thread_id})..."
    )
    ...
    print(
        f"   ✅ Pause Hit at Interrupt. Routed Path: '{routing_path}' (Expected: '{expected_path}')"
    )
    assert (
        routing_path == expected_path
    ), f"Expected routing path '{expected_path}', got '{routing_path}'."

    # Validate node executions based on route
    if expected_path == "escalation":
        assert values.get(
            "reasoning_output"
        ), "Escalation path should execute ReasoningAgent."
        assert values.get(
            "recommendation_output"
        ), "Escalation path should execute RecommendationAgent."
    else:
        assert values.get(
            "reasoning_output"
        ), "Standard path should populate stub reasoning."
        assert values.get(
            "recommendation_output"
        ), "Standard path should generate routine actions."

    assert values.get(
        "explanation_output"
    ), "Explanation node should execute on both paths."

    # Print action details
    rec = values["explanation_output"]
    selected = rec["selected_action"]
    print(
        f"   💡 Recommendation generated: '{selected['title']}' (Confidence: {rec['computed_confidence']['score']:.2f})"
    )

    # 5. Submit human feedback & resume
    print("   Step 2: Submitting Human Approval...")
    human_feedback = {
        "human_feedback": {
            "outcome": "approved",
    ...
    print(
        f"   ✅ Resumption complete. Outcome logged to SQLite (Feedback ID: {outcome_id})"
    )
