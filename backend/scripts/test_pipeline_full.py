...
def main():
    ...
    print(SEPARATOR)
    print(
        "  AGENTIC DECISION INTELLIGENCE PLATFORM — FULL 5-AGENT PIPELINE TEST (SHIFT 5)"
    )
    print(SEPARATOR)
    ...
    print(
        f"   Updated computed historical acceptance rate: {new_conf['historical_acceptance_rate']:.2f} (based on SQLite records: 1/3 approved)"
    )
    print(f"   New computed confidence score: {new_conf['score']:.2f}")
    assert (
        new_conf["historical_acceptance_rate"] == 0.33
    ), "Confidence logic should calculate 1/3 (0.33) acceptance rate."
    ...
    if "learned_heuristics" in playbook_ids:
        ...
        # Print excerpt
        heuristic_pb = next(
            pb for pb in context_loop["playbooks"] if pb["id"] == "learned_heuristics"
        )
