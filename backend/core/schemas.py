...
class DecisionPoint(BaseModel):
    ...
    description: str = Field(
        ..., description="Detailed description of what this decision point represents"
    )
    trigger_conditions: List[str] = Field(
        default_factory=list, description="Conditions that trigger this decision point"
    )
    priority: int = Field(
        default=1, description="Priority level of the decision point (1-5)"
    )
...
class DomainPack(BaseModel):
    ...
    description: str = Field(
        ..., description="Description of the domain pack's scope and purpose"
    )
    entities: List[str] = Field(
        default_factory=list, description="List of domain-specific entity names"
    )
    workflows: List[str] = Field(
        default_factory=list, description="Workflows supported within this domain"
    )
    business_rules: List[Dict[str, Any]] = Field(
        default_factory=list, description="JSON configuration of business rules"
    )
    success_metrics: List[str] = Field(
        default_factory=list,
        description="KPIs and success metrics measured in this domain",
    )
    tools: List[str] = Field(
        default_factory=list, description="Names of tools this pack utilizes"
    )
    prompt_overrides: Dict[str, str] = Field(
        default_factory=dict,
        description="Overrides for system prompt templates per agent",
    )
    decision_points: List[Union[str, DecisionPoint]] = Field(
        default_factory=list, description="Decision points within this domain"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional domain metadata"
    )
...
class AgentSpec(BaseModel):
    ...
    capabilities: List[str] = Field(
        default_factory=list, description="List of tasks this agent can execute"
    )
    input_schema: Dict[str, Any] = Field(
        default_factory=dict, description="JSON schema for valid input to this agent"
    )
    output_schema: Dict[str, Any] = Field(
        default_factory=dict, description="JSON schema for agent output"
    )
...
class ComputedConfidence(BaseModel):
    ...
    source_agreement: float = Field(
        ..., description="Ratio of consensus across evidence sources (0.0 to 1.0)"
    )
    historical_acceptance_rate: float = Field(
        ..., description="Historical human acceptance rate of this recommendation type"
    )
    confidence_band: Optional[str] = Field(
        "Medium Confidence",
        description="Calibrated confidence band (High, Medium, Low)",
    )
    confidence_reason: Optional[Dict[str, float]] = Field(
        None, description="Granular components of the confidence score calculation"
    )
...
class EvidenceNode(BaseModel):
    ...
    source: str = Field(
        ..., description="The source ID, name, or filename of the evidence"
    )
    source_type: str = Field(
        ..., description="Type of the source (e.g. email, CRM note, ticket, playbook)"
    )
    content: str = Field(
        ..., description="Snippet, content, or summary of the evidence details"
    )
    confidence: float = Field(
        default=1.0,
        description="Factual reliability or confidence score of this source (0.0 to 1.0)",
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Arbitrary metadata (e.g., recency, author)"
    )
...
class CandidateAction(BaseModel):
    ...
    rationale: str = Field(
        ..., description="The logical reasoning for suggesting this action"
    )
    expected_impact: str = Field(
        ..., description="Expected business outcomes of executing this action"
    )
    confidence: float = Field(
        ..., description="Confidence score associated with this option (0.0 to 1.0)"
    )
    rejected_reason: Optional[str] = Field(
        None,
        description="Detailed reason for rejection if this action was not selected as primary",
    )
...
class Recommendation(BaseModel):
    ...
    entity_id: str = Field(
        ...,
        description="Identifier for the target entity (e.g., account ID or candidate ID)",
    )
    domain_pack_id: str = Field(..., description="Identifier of the domain pack used")
    candidate_actions: List[CandidateAction] = Field(
        default_factory=list, description="Considered candidate action options"
    )
    selected_action: Optional[CandidateAction] = Field(
        None, description="The selected action chosen as next best action"
    )
    evidence: List[EvidenceNode] = Field(
        default_factory=list, description="Evidence nodes collected during evaluation"
    )
    recommendation_sources: List[str] = Field(
        default_factory=list,
        description="Explicit record of sources (playbooks/past cases) that drove this action",
    )
    reasoning_trace: List[str] = Field(
        default_factory=list, description="Internal agent chain of thought trace"
    )
    computed_confidence: ComputedConfidence = Field(
        ..., description="Calculated confidence score details"
    )
    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                ...
                "evidence": [],
                "reasoning_trace": [
                    "Context Agent: retrieved 2 playbooks",
                    "Reasoning Agent: renewal risk high",
                ],
                "computed_confidence": {
                    "score": 0.88,
                    "evidence_count": 5,
...
class MemoryWrite(BaseModel):
    ...
    interaction: str = Field(
        ..., description="Details of the interaction that triggered the run"
    )
    recommendation: Recommendation = Field(
        ..., description="The recommendation that was shown to the human"
    )
    human_feedback: Optional[str] = Field(
        None, description="Feedback text left by the human operator"
    )
    outcome: str = Field(
        ..., description="Outcome of human approval (e.g. approved, edited, rejected)"
    )
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="Timestamp when this interaction was completed",
    )
